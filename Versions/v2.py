...
##        cat_size = dp.read_size(self.data_path+"categories_score.tsv", group_lists,1)
 ...
 #-------------------------------Beginning of Algorithm-----------------------------------------------
        ...
        # Hyper Parameters
        thresholds = [[0.5,0.5,0.5,0.5],[0.90, 0.85, 0.85, 0.80]]
        minGroups = 25
        x = 3
        ...
        for i in range(len(c_score)):
            popScore[i] = 0/math.log2(len(self.list_map[group_lists[i]]))
        ...
        for _ in range(v):
                ...
                for i in topGroups:
                    ...
                    for j in peerGroups[i[1]]:
                        ...
                        if(len(peerGroups[i[1]])!=1):
                            # betaScore /= ((len(peerGroups[i[1]])-1)*len(peerGroups[i[1]]))
            ...
            for k in topKGroups:
                ...
                for ent in entity_list[i]:
                    ...
                    entScore += (alphaScores[ent]/(len(peerGroups[i])+1)) 
                    for p in peerGroups[i]:
                        entScore += ((2*beta)/(len(peerGroups[i])*(len(peerGroups[i])+1))) * relMat[p[1]][ent]
                ...
                # topKGroups.remove(k)
                    ...
                    mex = 0
                topKGroups_new.append(tuple([((k[0]*n_ele)/(n_ele+1)) + mex, k[1]]))
            ...
            for i in topGroups:
                ...
                for ent in entity_list[i[1]]:
                    ...
                    entScore += (alphaScores[ent]/(len(peerGroups[i[1]])+1)) 
                    for p in peerGroups[i[1]]:
                        entScore += (beta/(len(peerGroups[i[1]])*(len(peerGroups[i[1]])+1))) * relMat[p[1]][ent]
                ...
                currScore = ((i[0]*n_ele)/(n_ele+1)) + mex
                for j in peerGroups[i[1]]:
                    kn = 0
                    gammaScore = 0
                    for d in topKGroups[:-1]:
            ...
            for i in topGroups:
                ...
                for j in peerGroups[i[1]]:
                    ...
                    sm += alphaScores[j[1]] / len(peerGroups[i[1]])
                groupScores[i[1]] = sm
        ...
        while True:
            ...
            for i in topGroups:
                ...
                for j in peerGroups[i[1]]:
                    ...
                    currScore += (alphaScores[j[1]]/len(peerGroups[i[1]])) + betaScore
                    betaScores[j[1]] = betaScore
                ...
                for j in peerGroups[i[1]]:
                    if (alphaScores[j[1]]/len(peerGroups[i[1]])) + betaScores[j[1]] < mexScore:
                        mexScore = (alphaScores[j[1]]/len(peerGroups[i[1]])) + betaScores[j[1]]
                        repl = j[1]
                ...
                for k in entity_list[i[1]]:
                    ...
                    newScore = currScore - (alphaScores[repl]/len(peerGroups[i[1]])) - (2*betaScores[repl])

                    newScore += (alphaScores[k]/len(peerGroups[i[1]]))
                    newBetaScore = 0
                    ...
                    if(len(peerGroups[i[1]])!=1):
                        newBetaScore /= ((len(peerGroups[i[1]])-1)*len(peerGroups[i[1]]))
